...
func Test_service_GetNodeInfoByID(t *testing.T) {

	type args struct {
		ctx     context.Context
		payload *model.GetNodeInfoPayload
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := svc.GetNodeInfoByID(tt.args.ctx, tt.args.payload)
			if (err != nil) != tt.wantErr {
				t.Errorf("service.GetNodeInfoByID() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("service.GetNodeInfoByID() = %v, want %v", got, tt.want)
			}
		})
	}
}

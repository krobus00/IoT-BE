...
func TestController_HandleGetPagination(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "WHEN get all nodes THEN system return paginate node",
			pageSize: "1",
			mockCustomValidator: &mockCustomValidator{
				res: true,
			},
		...
		{
			name:     "WHEN get all nodes AND payload malformated THEN system return error",
			pageSize: "aaa",
			mockCustomValidator: &mockCustomValidator{
				res: true,
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodGet, "/iot/nodes?"+q.Encode(), nil)
			...
			if tt.mockGetAllNodes != nil {
				mockNodeService.On("GetAllNodes", mock.Anything, mock.Anything).Return(tt.mockGetAllNodes.res, tt.mockGetAllNodes.err)
			}
			...
			err := handler.HandleGetPagination(c)

...
func (c *Controller) HandleDeleteSensor(eCtx echo.Context) error {
	ctx := eCtx.Request().Context()

	span := kro_util.StartTracing(ctx, tag, tracingDeleteSensor)
	defer span.Finish()

	payload := new(model.DeleteSensorRequest)
	...
	err := c.SensorService.DeleteSensorByID(ctx, payload)
	if err != nil {
		return err
	}

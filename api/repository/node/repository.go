...
type (
	NodeRepository interface {
		GetTableName() string
		Store(ctx context.Context, db kro_pkg.Querier, input *db_models.Node) error
		FindNodeByID(ctx context.Context, db kro_pkg.Querier, input *db_models.Node) (*db_models.Node, error)
		...
		CountNodes(ctx context.Context, db kro_pkg.Querier) (int64, error)
	}
	repository struct {
